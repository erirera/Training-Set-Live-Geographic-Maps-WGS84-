...
csv_pred = os.path.join(BASE, "../prediction set EDA/jura_prediction_set.csv")
csv_val  = os.path.join(BASE, "../../Validation set/validation set EDA/jura_validation_set.csv")
...
def convex_hull(points):
    ...
    for p in reversed(pts):
        while len(upper) >= 2 and _cross(upper[-2], upper[-1], p) <= 0:
            upper.pop()
        upper.append(p)
    return lower[:-1] + upper[:-1]   # closed polygon, without repeated endpoint

def offset_hull(hull, offset_km):
    """Expand each hull vertex outward from the centroid by offset_km."""
...
# ── Load ALL points (prediction + validation combined for the boundary) ──────
def load_xy(path):
    pts = []
...
all_xy = load_xy(csv_pred) + load_xy(csv_val)
hull_km   = convex_hull(all_xy)
hull_off  = offset_hull(hull_km, offset_km=0.5)

# ── Convert hull km → WGS84 for injection ───────────────────────────────────
...
# ── Load prediction rows for the map ─────────────────────────────────────────
rows = []
def clean(v): return float(v.replace(',','.').replace('/.','.').replace('..','.'))
with open(csv_pred, newline="") as f:
    reader = csv.reader(f)
    next(reader)
    for line in reader:
...
<title>Prediction Set &mdash; Live Geographic Maps (n=$n_samples)</title>
...
<header>
  ...
  <div class="header-text">
    ...
    <p>Prediction Set &middot; 7 Heavy Metals &middot; Projected on OpenStreetMap Topology</p>
  </div>
  <div class="nav-group">
    <div class="badge">n = $n_samples samples</div>
    <a href="../prediction set EDA/index.html" class="nav-btn">&larr; Back to EDA</a>
    <a href="../../Validation set/validation set map/index.html" class="nav-btn">Validation Maps &rarr;</a>
  </div>
</header>
<main>
...
<footer>Jura Spatial Maps &middot; Swiss Jura Heavy Metals Dataset &middot; Prediction Set (n=$n_samples)</footer>
<script>
var ROWS     = $rows_js;
var METALS   = $metals_js;
var BOUNDARY = $boundary_js;   // [[lat,lon], ...] WGS84 convex hull +1 km offset
...
METALS.forEach(function(m){
  ...
  ROWS.forEach(function(r){
    ...
    circle.bindPopup(
      '<b>Sample ID: '+r.id+'</b><hr style="border:0;border-top:1px solid #3a3a6a;margin:6px 0;">'+
...
print(f"Prediction Maps written -> {out}  (hull pts: {len(hull_km)}, offset hull pts: {len(hull_off)})")
